...
import { generateSimpleIdentifier, RoomInfo } from "@coup/shared";
...
export const rooms = {
  ...
  create: (
    ...
    for (let attempts = 0; attempts < 3; attempts++) {
      ...
      console.log(
        `Room [${id.toUpperCase()}] was created${
  ...
  list_info: (): RoomInfo[] => {
    return Object.keys(room_list).map(
      (room_id: string) => rooms.info(room_id) as RoomInfo,
    );
  },
  max: (): number => parseInt(process.env.ROOM_MAX_PLAYERS || "10"),
};
